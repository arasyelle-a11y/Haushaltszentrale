klappt nicht, irgendwas falsch?
da hat sich am Design noch nichts geändert
...
die seitlichen leisten könnten etwas breiter oder?



Bearbeiten

jetzt sind doppelt Nägel



Bearbeiten

jetzt ist es insgesamt verzehrt


Bearbeiten

da stimmt was noch nciht
fast aber kein holz
klappt, allerdings haben wir noch einen grauen Hintergrund 
sieht gut aus, nur noch text anpassen, ich würde da glaube ich das symbol gar nicht nehmen, nur den Text in einer etwas geschnörkelteren Schrift, aber noch gut lesbar



Bearbeiten

sieht gut aus, nur noch ein bisschen runter



Bearbeiten

ja aber er listet alle Artikel auf, nicht bloss die mit Mi
klappt, wenn ich auf Vorräte klicke nachdem ich in den Kategorien war, kommt:
merkwürdig ist das auf einmal auch die Haferflocken bei Getränken stehen
kann der status direkt mit in dem ovalen kasten stehen?



Bearbeiten

vielleicht machen wir die Lagerorte noch kleiner, oder verborgen hinter einem drag over button oder so und die Mengenänderung in der gleichen Zeile wie den Namen



Bearbeiten

ich habe es doppelt, und ich möchte doch lieber ohne aufklappen, einfach eine kleine Zeile mit dem Text sieht ruhiger aus 

app-2.js
JavaScript
ich hab was falsch gemacht, jetzt wird gar nichts mehr geladen
so sieht es auf dem handy aus 
heute 7:24
Irgendwie sieht es auf dem Handy noch sperrig aus. 
Worked for 1m 3s


Bearbeiten

die Kachel sollte eigentlich genauso groß sein wie das brett
png zuschneiden denke ich, oder?


Bearbeiten

Ja — das Problem lag sehr wahrscheinlich an der großen Bildfläche rund um das Brett. Ich habe die PNG jetzt eng auf den Brettrand zugeschnitten und die abgerundeten Ecken transparent gemacht.
wood-board-cropped.png

Lade sie in GitHub hoch und ersetze damit deine bisherige wood-board.png. Falls der Browser noch die alte Version zeigt, ändere kurz in styles.css:

background-image: url("wood-board.png?v=2");
Dann sollte das sichtbare Brett tatsächlich die komplette Größe der Vorratskarte einnehmen. 
...
es hatte sich noch nicht richtig aktualisiert. ich mache noch einen screenshot wie es jetzt aussieht, bevor wie Änderungen machen. 
so sieht es aus jetzt aus, wenn das Fenster am Mac ganz geöffnet ist.
die Handyansicht wird auch die wichtigere sein, da wir das ja eher am Handy aktualisieren 
...
styles(1).css
Datei
kannst du hier auch mal aufräumen und prüfen?
Ist das schon die dünnste Version?
...
der mittlere Teil sieht unordentlich aus. wie kann man grafisch die Informationen besonders schön anordnen
...
index(1).html
Datei
Stimmt — die Download-Anzeige ist wieder nicht erschienen. Die Dateien sind aber vorhanden. Hier noch einmal einzeln:
index-final.html
...
schick mal die app.js codę als text
55s nachgedacht
Die komplette app.js hat 2.188 Zeilen. In einer einzigen Chatnachricht würde sie sehr wahrscheinlich abgeschnitten und wäre anschließend unbrauchbar.
Hier ist sie deshalb als direkt öffnbarer Textlink:

app-final.js

Beim Öffnen solltest du den vollständigen Code sehen und kannst alles mit ⌘ A und ⌘ C kopieren.

app-final.js
Code

da kommt immer: unsupported media file
Ah, deshalb klappt es nicht: Die .js-Datei wird bei dir offenbar als Mediendatei behandelt.
Ich habe sie deshalb als normale Textdatei gespeichert:

app-final.txt

Darin steht der vollständige app.js-Code. Du kannst alles mit ⌘ A und ⌘ C kopieren und in GitHub in app.js einfügen.
